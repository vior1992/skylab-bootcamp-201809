...
(function() {
    ...
    (function() {
        ...
        tests.push(function() {
            ...
            var lastPos = ayay.length;
            var lastValue = ayay[ayay.length-1];
            var result = ayay.pop();

    
            if (result !== lastValue) throw Error('you have deleted the wrong number');
            if (ayay.length == lastPos) throw Error('the last position has not been erased');
