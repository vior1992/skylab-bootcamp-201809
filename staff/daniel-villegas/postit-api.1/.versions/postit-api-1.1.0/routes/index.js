...
router.patch('/users/:id/profile', [bearerTokenParser, jwtVerifier, jsonBodyParser], (req, res) => {
    routeHandler(() => {
        const { body: {name, surname, newPassword, repeatNewPassword, password}, params: { id }, sub} = req
        
        if (id !== sub) throw Error('token sub does not match user id')

        return logic.updateProfile(id, name, surname, newPassword, repeatNewPassword, password)
            .then(() =>
                res.status(201),
                res.json({
                    data: user
