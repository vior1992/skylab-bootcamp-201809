function getQuestions()
		{
			return {
			    ...
			    m:[{answer: "misantropo", question:"CON LA M. Persona que huye del trato con otras personas o siente gran aversión hacia ellas", done:false}, {answer: "mediania", question:"Lugar medio entre dos partes o extremos.", done:false}, {answer: "madrid", question:"CON LA M. Capital de España", done:false}],
		   ...
		   function Player(_name)
		   {
		   		...
		   		this.setDone = function(letter, index){

					if (!this.questions[letter][index].done)
		   				this.questions[letter][index].done = true;
		   		};
		   		this.FindQuestion = function(letter){
		   		
		   			var question;
		   			question = this.questions[letter].find(x => x.done === false);
		   			return question;

		   		};
		   		this.CheckAnswer = function(letter, answer, question){
		   			...
		   			{
		   				if (this.questions[letter].filter(x => x.done).length === 3)
		   				{
		   					this.letters[letter] = "error";
		   				} 
		   				return; 
		   			}
		   		...
		   		this.DrawRound = function(){
		   			...
		   			console.log("");
		   			console.log("El jugador -> " + this.name + " ha obtenido " + this.points + " punto/os quedando la ronda de la siguiente manera:");
			    	console.log("");
		   			for(var letter in this.letters)
		   			{
		   				switch(this.letters[letter])
		   				{
		   					case "ok":
		   						console.log("%c " + letter.toString().toUpperCase() + " -> " + this.letters[letter], "color:green;");
		   					break;
		   					case "error":
		   						console.log("%c " + letter.toString().toUpperCase() + " -> " + this.letters[letter], "color:red;");
		   					break;
		   					default:
		   						console.log("%c " + letter.toString().toUpperCase() + " -> " + this.letters[letter], "color:blue;");
		   		...
		   		this.ProcessRound = function(num_ronda = 1, repeticion = false){

			    	var question;
			       	var resp = "";
			    	var repetir_ronda = false;
			    	
			    	if (!repeticion){

			    		console.log("Comienza la ronda " + num_ronda + " de preguntas para el jugador " + this.name);
			    		console.log("-------------------------------------------------------------------------------");
			    		console.log("");
			    	
			    	}else{

			    		console.log(num_ronda + "ª ronda de preguntas dado que el jugador " + this.name + " pasó palabras en la ronda anterior");
			    		console.log("");
			    	}
			    	...
			    	for(var letter in this.letters){
						...
						{
							question = this.FindQuestion(letter);
							resp = prompt(question.question);
							if ((resp === null) || (resp === "")){

								resp = "pasapalabra";
							}
							if (resp.toLowerCase() === 'end'){

								return "end";
							}
							this.CheckAnswer(letter, resp, question);
						}
					}
					for(var letter in this.letters){
					
						repetir_ronda = (this.letters[letter] === "") || false;

						if (repetir_ronda)
						{
							num_ronda++;
							this.ProcessRound(num_ronda, true);
						}
					}
		    ...
		    function GetPlayers(num_players, players){

		    	for(var i = 1; i <= num_players; i++)
					{
						players.push(new Player(prompt("Introduzca el nombre del jugador " + i)));
					}
		    }
		    ...
		    function ShowRanking(players){
		    	...
		    	console.log("");
		    	console.log("");
		    	console.log("EL RANKING QUEDA DE LA SIGUIENTE MANERA");
		    	console.log("----------------------------------------");
		       	console.log("");
		    	var ordered = players.sort((a,b) => b.points - a.points);
		    	for(var i = 0; i < ordered.length; i++)
				{
					console.log((i + 1) + "º " + ordered[i].name);
				}
		    }
			...
			function MainProgram(){
				...
				console.log("--------");	
				console.log("");
				console.log("Existen tres preguntas por letra, si el jugador no responde o responde pasapalabra, en una siguiente ronda, las letras");
				console.log("que han quedado pendientes por contestar serán preguntadas de nuevo al jugador con nuevas preguntas hasta un máximo de tres rondas (pq hay tres preguntas)");
				console.log("El control del tiempo no ha podido ser implementado dado que mientras se muestra un prompt para que el usuario introduzca la respuesta");
				console.log(", dicho prompt bloquea el hilo principal de ejecución, con lo que si tenemos un contador temporal mientras el prompt se muestre, este no se estára ejecutando");
				console.log("con lo que no nos sirve de nada.");
				console.log("");
				console.log("");
				do
				{
					...
					num_players = null;
					resp = "";
					num_players = prompt("¿Cuantos jugadores quiere que participen...?");
					if ((num_players === null) || (num_players === "") || (isNaN(num_players)))
					{
						alert("Tiene que introducir un numero entero, saliendo del programa...");
						break;
					}
					GetPlayers(num_players, players);
					for(var i = 0; i < players.length; i++){

						resp = players[i].ProcessRound();
						if (resp === "end")
						{
							break;
						}
						if (resp === "time_expired")
						{
							console.log("Tiempo expirado para el jugador -> " + players[i].name);
						}
					}
					if (resp !== "end")
					{
