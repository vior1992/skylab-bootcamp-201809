		...
		function sayItWithWords1(num)
		{
			...
			if ((num >= 10) && (num <= 19))
			{
				result_nums.push(Number.parseInt(nums[0]) * 10);
				result_nums.push(Number.parseInt(nums[1]));
				return teens[Number.parseInt(num.toString().split("")[1])] + ", " +  result_nums.join(" + ") + " / " + num;
			}
			...
			return result.join("-") + ", " + result.join(" + ") + " / " + result_nums.join(" + ");
		...
		function Replace(_texto)
		{
			...
			for(var p in dic)
			{
				...
				_texto = _texto.replace(regex, dic[p]);
			}
			return _texto;
		}			
		...
		function Replace1(_texto)
		{
			...
			{
				regex = new RegExp(p.toString(), "gmi");
				console.log("La palabra " + p + " tiene " + _texto.match(regex).length + " coincidencias");
				_texto = _texto.replace(regex, dic[p]);
			}
			console.log("");
			return _texto;
